...
const DeleteConfirmModal = ({ profile, onClose, onConfirm }: { profile: SpeakerProfile, onClose: () => void, onConfirm: () => void }) => {
...
export default function AdminSpeakerDashboard() {
    ...
    const [selectedProfileForDelete, setSelectedProfileForDelete] = useState<SpeakerProfile | null>(null);
    ...
    const exportToExcel = () => {
        ...
        const exportData = filteredProfiles
            ...
            .map((p, index) => ({
                ...
                'Company': p.company,
                'Speaker Bio': p.speaker_bio,
                'Topic Title': p.topic_title,
                'Topic Abstract': p.topic_abstract,
        ...
        XLSX.writeFile(workbook, `Speakers_Detailed_Export_${new Date().toISOString().split('T')[0]}.xlsx`);
    };
