...
const translations = {
  en: {
    title: 'Vision and Strategic Goals',
    ...
    goals: [
      {
        ...
        desc: 'Provide startups with capital and visibility to scale their innovations and bring transformative solutions to market.',
      },
      {
        icon: HandshakeIcon,
        title: 'Connect Investors & Startups',
        desc: 'Facilitate meaningful connections between high-net-worth investors and high-potential ventures for mutual growth.',
      },
      {
        icon: GlobeIcon,
        title: 'Promote UAE as Innovation Hub',
        desc: 'Establish the UAE as a global center for innovation and entrepreneurship, attracting talent and investment worldwide.',
      },
      {
        icon: UsersIcon,
        title: 'Build Annual Ecosystem',
        desc: 'Create a sustainable network of investors, mentors, and innovators that persists beyond the summit year.',
      },
    ],
  },
  ar: {
    ...
    goals: [
      {
        ...
        desc: 'تزويد الشركات الناشئة برأس المال والرؤية لتوسيع ابتكاراتها وتقديم حلول تحويلية إلى السوق.',
      },
      {
        icon: HandshakeIcon,
        title: 'ربط المستثمرين والشركات الناشئة',
        desc: 'تسهيل الاتصالات الهادفة بين المستثمرين ذوي الملاءة العالية والمشاريع الواعدة لتحقيق النمو المتبادل.',
      },
      {
        icon: GlobeIcon,
        title: 'تعزيز مكانة الإمارات كمركز للابتكار',
        desc: 'تأسيس دولة الإمارات العربية المتحدة كمركز عالمي للابتكار وريادة الأعمال، وجذب المواهب والاستثمار من جميع أنحاء العالم.',
      },
      {
        icon: UsersIcon,
        title: 'بناء منظومة سنوية',
        desc: 'إنشاء شبكة مستدامة من المستثمرين والموجهين والمبتكرين تستمر إلى ما بعد عام القمة.',
      },
    ],
  },
}
...
export default function VisionAndGoals({ language = 'en' }: VisionAndGoalsProps) {
  ...
  // Effect to manage scroll and resize events
  useEffect(() => {
    const el = scrollContainerRef.current
    if (el) {
  ...
  return (
    <section 
      // ADJUSTMENT 1: Decreased vertical padding (py-32 -> py-24 on desktop)
      className="relative py-12 sm:py-16 lg:py-24 px-4 sm:px-6 lg:px-8 bg-slate-900 text-white overflow-hidden"
    >
      {/* Background Effect */}
      <div className="absolute inset-0 opacity-10 pointer-events-none">
        <div className="absolute -top-1/4 -left-1/4 w-1/2 h-1/2 bg-blue-600 rounded-full blur-[150px]" />
      </div>
      ...
      <div className="max-w-7xl mx-auto relative z-10 w-full">
        ...
        <div className="text-center mb-16 lg:mb-20">
          ...
          <div className="mt-6 h-1.5 w-20 bg-gradient-to-r from-cyan-400 to-blue-500 rounded-full mx-auto" />
        </div>

        {/* Vision Section (Refined Typography) */}
        <div className="max-w-4xl mx-auto mb-20 p-6 sm:p-8 rounded-2xl border border-slate-700 bg-slate-800 shadow-2xl shadow-blue-500/10">
          ...
          <p className="mt-2 text-base sm:text-lg text-center text-slate-300 font-light italic">
            {t.visionDesc}
          </p>
        </div>
        ...
        {/* --- Strategic Goals Mobile Carousel Wrapper --- */}
        <div className="relative -mx-4 px-4 md:mx-0 md:px-0">
          
          {/* Left Arrow */}
          <button
            onClick={() => scroll('left')}
            className={`absolute top-1/2 -translate-y-1/2 left-0 z-10 p-3 bg-slate-700/80 backdrop-blur-sm rounded-full shadow-lg text-white hover:bg-slate-700 transition-opacity
          ...
          <div
            ...
            className="flex flex-nowrap overflow-x-auto scroll-smooth snap-x snap-mandatory gap-6 px-2 py-4
                       md:grid md:grid-cols-2 md:gap-8 md:p-0 md:overflow-visible // Grid restored on medium screens
          ...
          >
            {t.goals.map((goal, index) => {
              ...
              return (
                <div
                  key={goal.title}
                  // ADJUSTMENT 2: Decreased Card Padding (p-8 -> p-6 on desktop)
                  className="bg-slate-800 p-5 sm:p-6 rounded-2xl shadow-xl border border-slate-700 
                             w-[85vw] flex-shrink-0 snap-start
                             md:w-auto md:flex-shrink-1 md:col-span-1
                             transition-all duration-300 hover:shadow-cyan-400/20 hover:border-cyan-500"
                >
                  <div className="flex items-start space-x-4">
                    ...
                    {/* Text Content (Decreased Line Height) */}
                    <div>
                      <h4 className="text-base font-semibold mb-2 text-white">
                        {goal.title}
                      </h4>
                      <p 
                        // ADJUSTMENT 3: Tighter leading (leading-tight)
                        className="text-xs text-slate-400 font-light leading-tight"
                      >
                        {goal.desc}
                      </p>
                    </div>
          ...
          <button
            ...
            className={`absolute top-1/2 -translate-y-1/2 right-0 z-10 p-3 bg-slate-700/80 backdrop-blur-sm rounded-full shadow-lg text-white hover:bg-slate-700 transition-opacity
      ...
      <div className="absolute bottom-4 right-4 text-xs text-slate-500">
        Investarise Global Investor Summit - 2026: Shaping Tomorrow's Economy
      </div>
    </section>
  )
}